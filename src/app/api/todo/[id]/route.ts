...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  const todo = await prisma.todo.findUnique({ where: { id: Number(params.id) } }); //findUnique: 한개만 가져옴
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await prisma.todo.update({ //update
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  const deleted = await prisma.todo.delete({ where: { id: Number(params.id) } }); //delete
