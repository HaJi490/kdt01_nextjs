...
export default function Lotto() {
    ...
    const handleLottoNum = () =>{
        ...
        while(lottoNum.length < 7){
            const n:number = Math.floor(Math.random()*45) + 1; 
        ...
        //보너스 번호 
        const bonus:number[] = lottoNum.splice(-1); //❗splice #배열 결과 
       ...
       //TailBall 만들기
       const tm = lottoNum2.map((item:number | string) => item ==='+' ? <span key='nplus' className="w-16 h-16 flex justify-center items-center text-4xl font-bold mr-5"> {item} </span>
